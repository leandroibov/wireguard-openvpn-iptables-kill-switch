...
#in qubes is not necessary use variable $network, cause the default is eth0
block_input_usbrj45()
{
...
sudo iptables -A INPUT -p tcp --syn -s eth0 -j ACCEPT
...
#about line: 
#ip_info=$(ip addr show eth0 | grep 'inet ' | awk '{print $2}')
#in qubes the default is eth0, is not necessary ask primary network for $network variable
block_input()
{
...
erase_block_input_rules
    ...
    # Reject traffic that tries to leave via the physical NIC
    iptables -A OUTPUT -o eth0 -j REJECT --reject-with icmp-host-unreachable
...
#default in qubes is tun0, it is not necessary here
#echo
...
erase_block_input_rules
    ...
    #treating openvpn tun connection for kill switch
#unecessary cause Qubes has tun0 by default for any openvpn connection
#echo
#echo "Choosing virtual network interface to kill switch (tun0, tun1, tun2...):"
#conn=$(nmcli connection show | awk '{print $1}' | tail -n +2)
#echo "$conn"
